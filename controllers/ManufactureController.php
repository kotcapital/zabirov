...
class ManufactureController extends Controller
{
    ...
    public function actionDownload()
    {
		...
		return Excel::export([
            ...
            'fileName' => $table,
            'models' => $resarr,
            'columns' => ['manufacture_id', 'name', 'description', 'sys_id', 'title', 'keyword', ],
            'headers' => ['manufacture_id'  => 'manufacture_id', 'name'  => 'name', 'description'  => 'description', 'sys_id'  => 'sys_id', 'title'  => 'title', 'keyword'  => 'keyword', ],
        ]);
    }
	...
	public function actionUploadimg()
    {
        $model = new Upload();
		$res = "";
		$manufacture = Manufacture::findOne(Yii::$app->request->post('manufacture_id'));
        if ($manufacture != null) {
			$model->load(Yii::$app->request->post());
            $model->file = UploadedFile::getInstance($model, 'file');
			$model->file->saveAs('/var/www/html/basic/web/img/manufacture/' . $manufacture->manufacture_id . '.' . $model->file->extension);

			$manufacture->img = $model->file->extension;
			$manufacture->save();
        }
        return $this->redirect(['index']);
    }
    ...
    private function updateRecord($v){
        ...
        if ($v != null && is_array($v)) {
            ...
            if ($isset == self::RES_TRUE) {
                $model = Manufacture::findOne($v['manufacture_id']);
                if ($model == null) {
                    $model = new Manufacture();
                }
	...
	private function issetParams(array $array){
		$this->error = [];
		$nameArr = ['manufacture_id', 'name', 'description', 'sys_id', 'title', 'keyword', ];
