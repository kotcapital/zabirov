...
class SubcategoryController extends Controller
{
    ...
    public function actionDownload()
    {
		...
		return Excel::export([
            ...
            'fileName' => $table,
            'models' => $resarr,
            'columns' => ['subcategory_id', 'name', 'description', 'sys_id', 'title', 'keyword', 'category_id', 'middle_id', ],
            'headers' => ['subcategory_id'  => 'subcategory_id', 'name'  => 'name', 'description'  => 'description', 'sys_id'  => 'sys_id', 'title'  => 'title', 'keyword'  => 'keyword', 'category_id'  => 'category_id', 'middle_id'  => 'middle_id', ],
        ]);
    }
	...
	public function actionUploadimg()
    {
        $model = new Upload();
		$res = "";
		$subcategory = Subcategory::findOne(Yii::$app->request->post('subcategory_id'));
        if ($subcategory != null) {
			$model->load(Yii::$app->request->post());
            $model->file = UploadedFile::getInstance($model, 'file');
			$model->file->saveAs('/var/www/html/basic/web/img/subcategory/' . $subcategory->subcategory_id . '.' . $model->file->extension);

			$subcategory->img = $model->file->extension;
			$subcategory->save();
        }
        return $this->redirect(['index']);
    }
    ...
    private function updateRecord($v){
        ...
        if ($v != null && is_array($v)) {
            ...
            if ($isset == self::RES_TRUE) {
                $model = Subcategory::findOne($v['subcategory_id']);
                if ($model == null) {
                    $model = new Subcategory();
                }
	...
	private function issetParams(array $array){
		$this->error = [];
		$nameArr = ['subcategory_id', 'name', 'description', 'sys_id', 'title', 'keyword', 'category_id', 'middle_id', ];
